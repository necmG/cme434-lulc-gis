...
function getRiskLabel(p) {
  return p > 0.8 ? 'ÇOK YÜKSEK' :
         p > 0.6 ? 'YÜKSEK'     :
         p > 0.4 ? 'ORTA'       :
         p > 0.2 ? 'DÜŞÜK'      : 'ÇOK DÜŞÜK';
}
...
function showInfoCard(props) {
  ...
  body.innerHTML =
    row('Değişim Olasılığı', '<span style="color:' + riskColor + ';font-size:16px;font-weight:800">' + (p * 100).toFixed(1) + '%</span>') +
    row('Risk Sınıfı',       '<span style="color:' + riskColor + '">' + getRiskLabel(p) + '</span>') +
    ...
    row('Tahmin',            props.prediction === 1 ? '<span style="color:#BD0026">Değişmiş</span>' : '<span style="color:#4292C6">Değişmemiş</span>');
